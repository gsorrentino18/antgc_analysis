...
void optimizeCuts(){
	...
	TFile outFile("/local/cms/user/wadud/aNTGCmet/aNTGC_analysis/phoIDstudy/cutOptimize/sIetaIeta/sIetaIeta.root", "RECREATE");
	...
	while(inputTTreeReader.Next()){
		...
		std::vector<Double_t> vars({phoSigmaIEtaIEta_, phoHoverE_, phoPFClusEcalIsoCorr_, phoPFClusHcalIsoCorr_, phoTrkSumPtHollowConeDR03Corr_});
	...
	factory.BookMethod(&dataloader, TMVA::Types::kCuts, "CutsGA", "H:!V:VarProp[0]=FMin:VarProp[1]=FMin:VarProp[2]=FMin:VarProp[3]=FMin:VarProp[4]=FMin:FitMethod=GA:EffSel:PopSize=1000:Steps=60:Cycles=3:SC_steps=10:SC_rate=5:SC_factor=0.95:SaveBestCycle=15");
