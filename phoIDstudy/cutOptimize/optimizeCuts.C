...
void optimizeCuts(){
	...
	TTreeReaderAnyValue<Double_t>				bdtV2Score_										(inputTTreeReader, "bdtV2Score");
	...
	TFile outFile("trainingV2/cutOptmizationV2.root", "RECREATE");
	...
	TMVA::DataLoader dataloader("trainingV2/dataset");
	dataloader.AddVariable("bdtV2Score", "BDT Score", "", 'F');
	...
	while(inputTTreeReader.Next()){
		...
		std::vector<Double_t> vars({bdtV2Score_, phoHoverE_, phoPFECALClusIsoCorr_, phoPFHCALClusIsoCorr_, phoTkrIsoCorr_});
	...
	factory.BookMethod(&dataloader, TMVA::Types::kCuts, "CutsGA", "H:V:VerbosityLevel=Verbose:VarProp[0]=FMax:VarProp[1]=FMin:VarProp[2]=FMin:VarProp[3]=FMin:VarProp[4]=FMin:FitMethod=GA:EffSel:PopSize=600:Steps=50:Cycles=4:SC_steps=10:SC_rate=5:SC_factor=0.95:Seed=0");
