...
void getEffectiveArea(std::string _isoVar, ofstream* effAreaOutFile){
	...
	for(std::string etaStr : etaBins){
		...
		TH2F* 										histIsoVsRho							=	get2DHistogram(isoVar, etaMin, etaMax);
		std::string 								etaHistName								=	plotName + "_eta" + split_string(etaStr, ",")[0] +"to" + split_string(etaStr, ",")[1];
		TH1D* 										isoPcVSpT 								= 	histIsoVsRho->QuantilesX(percentile, etaHistName.c_str());
		...
		if(polOder == 0){
			...
			TF1* 									linFitCopy 									=  new TF1((etaHistName+"_fitLinearCopy").c_str(), linFitDef.c_str(), histIsoVsRho->GetXaxis()->GetXmin(), histIsoVsRho->GetXaxis()->GetXmax());
			...
			yMax 																				= 	std::max(yMax, linFitLine->Eval(histIsoVsRho->GetXaxis()->GetBinUpEdge(histIsoVsRho->GetNbinsX())));
			yMin 																				= 	std::min(yMin, linFitLine->Eval(histIsoVsRho->GetXaxis()->GetBinLowEdge(1)));
			...
			TF1* 									quadFitCopy 									=  new TF1((plotName+"_fitQuadCopy").c_str(), quadFitDef.c_str(), histIsoVsRho->GetXaxis()->GetXmin(), histIsoVsRho->GetXaxis()->GetXmax());
			...
			yMax 																				= std::max(yMax, quadFitLine->Eval(histIsoVsRho->GetXaxis()->GetBinUpEdge(histIsoVsRho->GetNbinsX())));
			yMin 																				= 	std::min(yMin, quadFitLine->Eval(histIsoVsRho->GetXaxis()->GetBinLowEdge(1)));
		}
