...
std::string 										optionsFile="plotFeatsOptions.txt";
...
void plotVar(std::string _plotVar, Float_t _etaMin, Float_t _etaMax, Float_t _pTmin, Float_t _pTmax){
	...
	while(inputTTreeReader.Next()){
		...
		if(isSignal_){
			signalHist.Fill(feat_, bdtWeight_);
		} else{
			backgroundHist.Fill(feat_, bdtWeight_);
		}
	}
	...
	// TLegend legend(options.getDouble("legW"), options.getDouble("legH"));
	...
	legend.AddEntry(&signalHist, "Prompt", "L");
	legend.AddEntry(&backgroundHist, "Fake", "L");
