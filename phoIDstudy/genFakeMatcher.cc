...
Bool_t genPhoMatcher::selectEvent(){
	...
	for(UShort_t iPho = 0; iPho < _nPho; iPho++){
		...
		Short_t iPhoMatchedGenPhoIndex = photonIsPrompt(iPho, 0.3, -0.4, 2.);
		if(iPhoMatchedGenPhoIndex < 0) continue;									// skip if no match with a prompt gen photon
		passedGenMatch =1;

		if(_phoCalibEt[iPho] > highestPtPhoPt){
			highestPtPhoPt = _phoCalibEt[iPho];
			highetsPtIndex = iPho;
			matchedGenPhoIndex = iPhoMatchedGenPhoIndex;
		}
	}
