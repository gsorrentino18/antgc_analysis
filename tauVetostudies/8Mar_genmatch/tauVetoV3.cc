	...
	while(inputTTreeReader.Next()){
		...
		if(isMC) {
		   ...
		   break;
               ...
               //taking only events with a prompt gen electron
               Bool_t hasPromptEle = 0;
               for(UShort_t iGenP=0; iGenP<_nMC; iGenP++){
                 if(std::abs(_mcPID[iGenP]) != 11) continue; //// e only
                 if(_mcStatus[iGenP] != 1) continue;     //// final state    
                 UShort_t iGenPStFl = _mcStatusFlag[iGenP];
                 if(!getBit(iGenPStFl,1)) continue;       //// isPromptFinalState()
                 hasPromptEle = 1;
                 break;
               }

               if(!hasPromptEle) continue;
