...
private:
	...
	Bool_t             photonPassingID(UShort_t phoIndex);
        Bool_t             isGenPhoMatched(Float_t _etaPho, Float_t _phiPho, Float_t _deltaRmax);
...
Bool_t tauVeto::isGenPhoMatched(Float_t _etaPho, Float_t _phiPho, Float_t _deltaRmax){

	Float_t minDeltaR = _deltaRmax;
	...
	for(UShort_t iGenP=0; iGenP < _nMC; iGenP++){
		...
		Float_t dR = deltaR(_etaPho, _phiPho, _mcEta[iGenP], _mcPhi[iGenP]);
	...
	while(inputTTreeReader.Next()){
		...
		if(current_entry % REPORT_EVERY == 0){
			...
			",\t\tevent\t"<<(_event)<<"\t\tFile " <<inputTree->GetCurrentFile()->GetName() <<std::endl;
                ...
                if (_pfMET < 170) continue; //high MET
		...
		if(isMC) {
		   ...
		   Char_t tmpeleIDbit = _eleIDbit[i];
	   	   if(!getBit(tmpeleIDbit, 1)) continue; 		// loose ID
		   setBit(lepVeto_,0,1);
                   nele++;
		   break;
	        }
                ...
                for(Int_t i = 0; i < _nTau; i++){
                     ...
                     if(!getBit(tmptauIDbitsDeepTau2017v2p1, 3)) continue; //byVVVLooseDeepTau2017v2p1VSjet
                     if(!getBit(tmptauIDbitsDeepTau2017v2p1, 11)) continue; //byVVVLooseDeepTau2017v2p1VSe
                     if(!getBit(tmptauIDbitsDeepTau2017v2p1, 17)) continue; //byVLooseDeepTau2017v2p1VSmu
               ...
               Bool_t foundSecondPho = false;

               Bool_t sourCou = false;
               Bool_t counted = false;
               for(UShort_t iPho=0; iPho<_nPho; iPho++){
                  ...
                  //if (!(isGenPhoMatched(_phoEta[iPho], _phoPhi[iPho], 0.5))) continue;

                  //if (_phoCalibEt[iPho] < 200) continue;

                  if (!sourCou) {
                     Char_t tmpPhoFiducialRegion = _phoFiducialRegion[0];
                     if(!getBit(tmpPhoFiducialRegion, 0)) continue; //EB photon
                  ...
                  if (!counted) {
                     ...
                     Char_t tmpPhoFiducialRegion = _phoFiducialRegion[iPho];
                     if ((_phoCalibEt[iPho] > 15.)) { //EB photon
                        if (getBit(tmpPhoFiducialRegion, 0)) {
                           criteriaEB_++;
                        }
                        counted=true;
                     }
                  ...
                  /*if (_phoCalibEt[iPho] < highestPt) {
                     if (_phoCalibEt[iPho] < 10.) continue;
                     if (_phoEta[iPho] > 2.5) continue;
                     Char_t tmpphoIDbit = _phoIDbit[iPho];
                     if(!getBit(tmpphoIDbit, 4)) continue; //requiring WP90
                     foundSecondPho = true;
                  }
               ...
               //if (foundSecondPho) continue; //veto event with second photon
...
void tauVeto::initEventType(eventType & evType, std::string typeName, std::string typeTitle){
	...
	evType.tree->Branch("minDeltaRPhoJet30", &minDeltaRPhoJet30_);
        ...
        evType.tree->Branch("totalEB", &totalEB_);
        evType.tree->Branch("criteriaEB", &criteriaEB_);
